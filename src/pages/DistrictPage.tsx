...
export default function DistrictPage() {
  ...
  const { places, loading, error } = useFilteredPlaces({ district: district || null });
  ...
  const districtDisplay = district ? district.charAt(0).toUpperCase() + district.slice(1) : "Район";
  const title = `Заведения района ${districtDisplay} | VIZIT AI`;
  const description = `Лучшие места района ${districtDisplay}: ${places.length} заведений — кафе, рестораны, барбершопы и другие. Найдите идеальное место с VIZIT AI.`;
  const canonicalUrl = `https://vizit-ai.vercel.app/district/${encodeURIComponent(district || "")}`;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Районы", url: `${canonicalUrl}` },
    ],
  };
      ...
      <Helmet>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            "@context": "https://schema.org",
            "@type": "ItemList",
            name: `Заведения района ${districtDisplay}`,
      ...
      <Layout>
        <div className="mx-auto max-w-3xl px-4 py-8">
          <nav className="mb-6 text-sm text-white/40">
            ...
            <span className="mx-2">/</span>
            <span className="text-white/60">{districtDisplay}</span>
          </nav>

          <h1 className="mb-2 text-2xl font-semibold text-[#ececec]">Заведения района {districtDisplay}</h1>
          <p className="mb-6 text-sm text-white/50">{places.length} мест в районе</p>
          ...
          {!loading && !error && places.length === 0 && (
            <div className="py-8 text-center">
              <p className="mb-4 text-sm text-white/40">
                Пока нет заведений в районе «{districtDisplay}».
              </p>
              <Link
                to="/vendor"
