...
export default function CategoryPage() {
  ...
  const { places, loading, error } = useFilteredPlaces({ category: category || null });
  ...
  const title = `${categoryLabel} в Астане | VIZIT AI`;
  const description = `Лучшие ${categoryLabel?.toLowerCase()} Астаны: ${places.length} мест. Найдите идеальное место с VIZIT AI.`;
  const canonicalUrl = `https://vizit-ai.vercel.app/category/${encodeURIComponent(category || "")}`;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Категории", url: `${canonicalUrl}` },
    ],
  };
    ...
    <>
      ...
      <Layout>
        <div className="mx-auto max-w-3xl px-4 py-8">
          ...
          <h1 className="mb-2 text-2xl font-semibold text-[#ececec]">{categoryLabel} в Астане</h1>
          <p className="mb-6 text-sm text-white/50">{places.length} мест в категории</p>
          ...
          {!loading && !error && places.length === 0 && (
            <div className="py-8 text-center">
              <p className="mb-4 text-sm text-white/40">
                Пока нет заведений в категории «{categoryLabel}».
              </p>
              <Link
                to="/vendor"
