...
export default function CityPage() {
  ...
  const { places, loading, error } = useFilteredPlaces({ city: city || null });
  ...
  const title = `Заведения ${cityDisplay} | VIZIT AI`;
  const description = `Лучшие места ${cityDisplay}: ${places.length} заведений — кафе, рестораны, барбершопы и другие. Найдите идеальное место с VIZIT AI.`;
  const canonicalUrl = `https://vizit-ai.vercel.app/city/${encodeURIComponent(city || "")}`;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Города", url: `${canonicalUrl}` },
    ],
  };
      ...
      <Layout>
        <div className="mx-auto max-w-3xl px-4 py-8">
          <nav className="mb-6 text-sm text-white/40">
            ...
            <span className="mx-2">/</span>
            <span className="text-white/60">{cityDisplay}</span>
          </nav>

          <h1 className="mb-2 text-2xl font-semibold text-[#ececec]">Заведения {cityDisplay}</h1>
          <p className="mb-6 text-sm text-white/50">{places.length} мест в городе</p>
          ...
          {!loading && !error && places.length === 0 && (
            <div className="py-8 text-center">
              <p className="mb-4 text-sm text-white/40">
                Пока нет заведений в городе «{cityDisplay}».
              </p>
              <Link
                to="/vendor"
